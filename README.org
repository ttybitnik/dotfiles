...
Before using my dotfiles, change the personal environment values in =.bash_profile= accordingly.
...
| TTY Environment     | GNU Emacs (ditto), GNU Midnight Commander, GNU Lynx, GNU Parallel, GNU Privacy Guard, ClamAV, Tmux, Git, Htop, Powertop, Hyperfine, Tree, Nmcli, Pulsemixer, Spotify-tui |
...
The scripts are located in the following repository path: =./.local/bin/=.
...
#+begin_src shell
  ...
  wayland_env() {
      ...
      create_symlink ".config/dunst/switcher-dark" ".config/dunst/dunstrc"
